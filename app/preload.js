...
exports.styleSettingsCss = exports.su = void 0;
const fs = require("fs");
...
electron_1.ipcRenderer.on('preloaduserscriptsPath', (event, recieved_userscriptsPath) => {
    exports.su.userscriptsPath = recieved_userscriptsPath;
    exports.su.userscriptTrackerPath = path.resolve(exports.su.userscriptsPath, "tracker.json");
    //init the userscripts (read, map and set up tracker)
    //remove all non .js files, map to {name, fullpath}
    exports.su.userscripts = fs.readdirSync(exports.su.userscriptsPath, { withFileTypes: true })
        .filter(entry => entry.name.endsWith(".js"))
        .map(entry => ({ name: entry.name, fullpath: path.resolve(exports.su.userscriptsPath, entry.name).toString() }));
    let tracker = {};
    exports.su.userscripts.forEach(u => tracker[u.name] = false); //fill tracker with falses, so new userscripts get added disabled
    Object.assign(tracker, JSON.parse(fs.readFileSync(exports.su.userscriptTrackerPath, { encoding: "utf-8" }))); //read and assign the tracker.json
    fs.writeFileSync(exports.su.userscriptTrackerPath, JSON.stringify(tracker, null, 2), { encoding: "utf-8" }); //save with the new userscripts
    exports.su.userscriptTracker = tracker;
    //run the code in the userscript
    exports.su.userscripts.forEach(u => {
...
electron_1.ipcRenderer.on('injectClientCss', (event, injectSplash, { hideAds, menuTimer }, userscripts, version) => {
    ...
    if (document.getElementById(settId) === null) {
        ...
        (0, utils_1.injectSettingsCss)(settCss, settId);
    }
    if (document.getElementById(splashId) === null && injectSplash === true) {
        ...
        (0, utils_1.injectSettingsCss)(splashCSS, splashId);
        const initLoader = document.getElementById("initLoader");
        if (initLoader === null) {
            throw "Krunker didn't create #initLoader";
        }
        initLoader.appendChild((0, utils_1.createElement)("svg", {
            id: "crankshaft-logo",
    ...
    if (hideAds) {
        (0, utils_1.toggleSettingCSS)(exports.styleSettingsCss.hideAds, "hideAds", true);
    }
    if (menuTimer) {
        (0, utils_1.toggleSettingCSS)(exports.styleSettingsCss.menuTimer, "menuTimer", true);
    }
    if (userscripts) {
        electron_1.ipcRenderer.send("preloadNeedsuserscriptsPath");
    }
});
/**
...
function UpdateSettingsTabs(activeTab, hookSearch = true) {
    ...
    try {
        clientTab.removeEventListener("click", settingsui_1.renderSettings);
    }
    catch (e) { }
    clientTab.addEventListener("click", settingsui_1.renderSettings);
